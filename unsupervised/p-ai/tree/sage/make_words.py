...
def choose_files(num_files, min_dpoints = 100):
    ...
    rand_files = [i for i in rand_files if len(ASASSN_Lightcurve.from_dat_file(i).times) >= min_dpoints]
    if len(rand_files) < num_files:
        rand_files.extend(choose_files(num_files,min_dpoints))
